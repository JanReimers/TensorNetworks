...
Hamiltonian_2Body_LongRange::Hamiltonian_2Body_LongRange(double S, double J, double hx, int NN)
    : itsS (S)
    , itsJ (J)
    , itshx(hx)
    , itsNN(NN)
...
Hamiltonian_2Body_LongRange::~Hamiltonian_2Body_LongRange()
{
...
MatrixOR  Hamiltonian_2Body_LongRange::GetW(MPOForm f) const
{
    ...
    case RegularLower:
        ...
        {
            W(iblock+1,0)=OperatorSz(itsS);
            for (int j=1;j<i;j++)
                W(iblock+1+j,iblock+j)=OperatorI(itsS);
            W(Dw-1,iblock+i)=itsJ/(i*i)*OperatorSz(itsS);
            iblock+=i;
        }
        break;
    case RegularUpper:
        ...
        for (int i=1;i<=itsNN;i++)
        {
            W(0,iblock+1)=OperatorSz(itsS);
            for (int j=1;j<i;j++)
                W(iblock+j,iblock+1+j)=OperatorI(itsS);
            W(iblock+i,Dw-1)=itsJ/(i*i)*OperatorSz(itsS);
            iblock+=i;
        }
        //cout << W << endl;
       break;
...
double Hamiltonian_2Body_LongRange::GetH(int ma,int na,int mb,int nb) const
{
